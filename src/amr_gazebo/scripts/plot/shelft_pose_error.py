...
class PlotTopic:
    ...
    def error_estimate(self, lst):
        ...
        return [dist2goal, error]

    def data_update(self, data):
        ''' data in Float64MultiArray [car_x, car_y, shelft_x, shelft_y] '''

        data_name = data.layout.dim[0].label.split(':')[-1]
        if len(self.shelft_dict) == 0 or not data_name in self.shelft_dict.keys():
            self.shelft_dict[data_name]=np.zeros((0,2))
        dist_error = self.error_estimate(data.data)
        self.shelft_dict[data_name] = np.append(
                                    self.shelft_dict[data_name], [dist_error], 0)
        print(self.shelft_dict)
    ...
    def setup_plot(self):
        ...
        self.ax0.set_title("Estimation Error Using ArUco w.r.t. Distance to Goal", fontsize=15)
        ...
        return self.scat0, self.scat1,

    def update(self, i):
        """Update the scatter plot."""
        if len(self.shelft_dict) != 0 and "202" in self.shelft_dict.keys():
            # Set x and y data... Nx2 n-D array
            self.scat0.set_offsets(self.shelft_dict["202"])
            self.scat1.set_offsets(self.shelft_dict["202"])
            # Set sizes...
